...
export default function CreateJobModal({ artisan, onClose }) {
  ...
  const modalHeader = useMemo(
    () => ({
      title: artisan ? `Hire ${artisan.name}` : "Post Public Job",
      isDirect: !!artisan,
    }),
    [artisan],
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md transition-opacity">
      <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border border-white/20 dark:border-slate-700/50 rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden animate-in fade-in zoom-in duration-300">
        ...
        {/* Form Body - Performance: Added 'contain-intrinsic-size' for scroll optimization */}
        <form
          onSubmit={handleSubmit}
          className="p-6 space-y-5 max-h-[75vh] overflow-y-auto custom-scrollbar overflow-x-hidden"
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={handleChange}
                disabled={modalHeader.isDirect}
                className="w-full px-4 py-3 bg-white/50 dark:bg-slate-800/50 border border-gray-300/50 dark:border-slate-600/50 rounded-xl dark:text-white disabled:opacity-50 appearance-none"
              >
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            <button
              type="button"
              onClick={onClose}
              className="order-2 sm:order-1 flex-1 px-4 py-4 rounded-2xl font-bold text-slate-600 dark:text-slate-400 hover:bg-black/5 dark:hover:bg-white/5 transition-all active:scale-95"
            >
              Cancel
            </button>
            <button
